...
if [ "$train_len" -eq 56 ]; then
    ...
    batch_size=1282 # Fill in the batch size for train_len=28
elif [ "$train_len" -eq 14 ]; then
    batch_size=  # Fill in the batch size for train_len=14
else
...
python -m run_training.train_past_guided \
        ...
        --loss_mae_weight 1 \
        --loss_mape_weight 20 \
        --output_dir output/past_guided/$(date +%m-%d-%H00)_$train_len-84/ \
